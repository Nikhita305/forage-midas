...
class SmartAmbulanceAPITester:
    ...
    def test_ambulance_endpoints(self):
        ...
        # Get ambulances
        success, ambulances = self.run_test("Get Ambulances", "GET", "ambulances", 200, token=self.token)
        
        if success and ambulances:
            ambulance_id = ambulances[0]['id']
            
            # Test ambulance status update
            success, _ = self.run_test("Update Ambulance Status", "POST", f"ambulance/{ambulance_id}/status?status=emergency", 200, token=self.token)
            ...
            success, _ = self.run_test("Update Ambulance Location", "POST", "ambulance/location", 200, location_data, token=self.token)
    ...
    def test_routing_endpoints(self):
        ...
        # First get ambulances to use one for routing
        success, ambulances = self.run_test("Get Ambulances for Routing", "GET", "ambulances", 200, token=self.token)
        ...
        if success and ambulances:
            ...
            success, _ = self.run_test("Request Route", "POST", "route/request", 200, route_data, token=self.token)
